...
for dir in "${!syncs[@]}"; do
  ...
  if [ ! -d "$dir_path" ]; then
    echo "The directory '$dir_path'"
    echo "Was not be found, skipping ⏩"
    continue
  fi

  rclone sync "$dir_path" "$REMOTE_PATH/$dir" "${RCLONE_FLAGS[@]}"
  print_status $? "$dir"
done
...
for dir in "${!backups[@]}"; do
  ...
  rclone copy "$dir_path" "$REMOTE_PATH/$dir" "${RCLONE_FLAGS[@]}"
  print_status
done
